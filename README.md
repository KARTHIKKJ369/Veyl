...
- **Lock-Free Concurrency**: Atomic ring-buffer scheduler for zero-drop audio rendering.
...
### 3. Jetpack Compose "Cockpit" UI (No AI-Slop)
...
- **Now Playing Cockpit**: High-density format badges (`FLAC 192kHz/24bit`, `BIT-PERFECT`, `AAUDIO MMAP`), 16-band live spectrum visualizer, millisecond precision scrub bar.
- **DSP Cockpit**: Live interactive frequency response curve, 10 parametric band controls, preamp fader, ReplayGain & Dither toggles.
- **Library & Search**: Filter by Hi-Res, Artists, Albums, Folders / OTG cards.
- **Playback Queue**: Gapless boundary indicators and reorderable track list.
- **Settings**: Toggle Bit-Perfect mode, MMAP exclusive stream, sinc resampler profiles, buffer tuning.
